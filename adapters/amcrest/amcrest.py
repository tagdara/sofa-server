...
class dlink(sofabase):
    ...
    class adapterProcess():
        ...
        async def start(self):
            ...
            await self.dataset.ingest({'camera': self.definitions.cameras})
        ...
        def getCaptureDirectories(self):
            ...
            try:
                result={}
                basepath = self.definitions.captures['basedir']
                
                for dirname in os.listdir(basepath):
                    fullpath = os.path.join(basepath, dirname)
                    if os.path.isdir(fullpath):
                        if dirname in self.definitions.cameras:
        ...
        def getCameraCaptureTypes(self, cameraname):
            ...
            try:
                ...
                campath = os.path.join(self.definitions.captures['basedir'], cameraname)
        ...
        def getCameraCaptureDates(self, cameraname, capturetype):
            
            try:
                result=[]

                campath = os.path.join(self.definitions.captures['basedir'], cameraname, capturetype)
        ...
        def getCameraCaptureHours(self, cameraname, capturetype, capturedate):
            ...
            try:
                ...
                precampath = os.path.join(self.definitions.captures['basedir'], cameraname, capturetype)
                # Motion capture events may use this structure
        ...
        async def virtualThumbnail(self, path, client=None):
            ...
            try:
                ...
                self.log.error('Couldnt get thumbnail image for %s (cancelled)' % path)
            except:
        ...
        async def virtualList(self, itempath):
            ...
            try:
                ...
                if '/' in itempath:
                    ...
                    if len(ip)==3:
                        if ip[1] in self.definitions.cameras:
