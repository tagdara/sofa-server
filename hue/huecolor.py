...
class ColorHelper:
    ...
    #  All of the following is a slightly modified version of 
    ...
    def check_point_in_lamps_reach(self, p):
        ...
        v1 = self.XYPoint(self.Lime.x - self.Red.x, self.Lime.y - self.Red.y)
        v2 = self.XYPoint(self.Blue.x - self.Red.x, self.Blue.y - self.Red.y)

        q = self.XYPoint(p.x - self.Red.x, p.y - self.Red.y)
    ...
    def get_closest_point_to_line(self, A, B, P):
        ...
        AP = self.XYPoint(P.x - A.x, P.y - A.y)
        AB = self.XYPoint(B.x - A.x, B.y - A.y)
        ...
        return self.XYPoint(A.x + AB.x * t, A.y + AB.y * t)
...
class colorConverter:
    ...
    def get_light_gamut(self, modelId):
        ...
        GamutA = (
            self.XYPoint(0.704, 0.296),
            self.XYPoint(0.2151, 0.7106),
            self.XYPoint(0.138, 0.08),
        )

        # Hue A19 bulbs
        GamutB = (
            self.XYPoint(0.675, 0.322),
            self.XYPoint(0.4091, 0.518),
            self.XYPoint(0.167, 0.04),
        )

        # Hue BR30, A19 (Gen 3), Hue Go, LightStrips plus
        GamutC = (
            self.XYPoint(0.692, 0.308),
            self.XYPoint(0.17, 0.7),
            self.XYPoint(0.153, 0.048),
        )
        ...
        if modelId in ('LST001', 'LLC010', 'LLC011', 'LLC012', 'LLC006', 'LLC007', 'LLC013'):
            ...
            return GamutB
        elif modelId in ('LCT010', 'LCT014', 'LCT011', 'LLC020', 'LST002', 'LCT015', 'LCT016'):
            return GamutC
        else:
            raise ValueError
        return None    
    ...
    def __init__(self, model='LCT001'):
        
        self.color=ColorHelper(self.get_light_gamut(model))
