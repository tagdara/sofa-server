...
FFMPEG_CMD = (
    # pylint: disable=bad-continuation
    'ffmpeg -re -threads 0 '
    '-rtsp_transport tcp -i rtsp://unifi-video.dayton.tech:7447/5cb78f92d34e80949e31bb2e_1 -f lavfi -i anullsrc -tune zerolatency -vcodec libx264'
    '-vcodec libx264 -an -pix_fmt yuv420p -r {fps} -f rawvideo -tune zerolatency '
    '-vf scale={width}:{height} -b:v {v_max_bitrate}k -bufsize {v_max_bitrate}k '
    '-payload_type 99 -ssrc {v_ssrc} -f rtp '
    '-srtp_out_suite AES_CM_128_HMAC_SHA1_80 -srtp_out_params {v_srtp_key} '
...
class Camera(Accessory):
    ...
    def get_supported_rtp_config(support_srtp):
        logging.warning('!! get_supported_rtp_config')
    ...
    def get_supported_video_stream_config(video_params):
        logging.warning('!! get_supported_video_stream_config')
    ...
    def __init__(self, options, *args, **kwargs):
        ...
        logging.error(self.start_stream_cmd)
        ...
        self.sessions = {}
        logging.error(self.stream_address)
    ...
    async def _start_stream(self, objs, reconfigure):  # pylint: disable=unused-argument
        ...
        """
        logging.warning('_start_stream: %s' % objs)
        ...
        logging.warning('_start_stream video_tlv: %s' % video_tlv)
    ...
    def set_selected_stream_configuration(self, value):
        ...
        :type value: ``str``
        """
        logging.warning('set_selected_stream_config - value - %s', value)
        ...
        logging.warning('Set stream session: %s', session)
        request_type = session[b'\x02'][0]
        logging.warning('Set stream config request: %d', request_type)
        if request_type == 0:
    ...
    async def start_stream(self, session_info, stream_config):
        ...
        :rtype: ``bool``
        """
        logging.warning('[%s] Starting stream with the following parameters: %s',
                      session_info['id'], stream_config)

        cmd = self.start_stream_cmd.format(**stream_config).split()
        logging.info('Executing start stream command: "%s"', ' '.join(cmd))
        try:
    ...
    def get_snapshot(self, image_size):  # pylint: disable=unused-argument, no-self-use
        ...
        #return await self.get_unifi_snap()
